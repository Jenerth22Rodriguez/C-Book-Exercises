...
static char *convert_number_to_binary(int, char *);
static void breakup_bits_into_groups(const char *);
...
char *convert_number_to_binary(int negative_number, char *ps)
{
   int index;
...
int invert_number(int number, int bits)
{
   int mask = 0;
   int bit_value = 1;
   while(bits-- > 0)
